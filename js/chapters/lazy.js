...
export default class LazyChapter extends THREE.Object3D {
      ...
      if (trigger == 'sceneB') {
        if (this.inSceneB) {
          ...
          TweenMax.to(this.cameraController, 2, {
            setX: followPosition.x,
            setY: followPosition.y,
            setZ: followPosition.z,
            currentFrustumSize: this.cameraController.defaultFrustumSize,
            onUpdate: () => {
  ...
  setup() {
    this.crumbOriginalQuaternion.setFromAxisAngle(
    ...
    for (let i = 0; i < this.colors.length; i++) {
      this.materials.push(
        new THREE.MeshLambertMaterial({ color: this.colors[i] }),
      )
    }
    ...
    this.crumbShape = this.ammo.createShapeFromBuffergeometry(
      this.crumbGeometry,
    )
    for (let i = 0; i < 20; i++) {
      for (let j = 0; j < 3; j++) {
        let _c = new THREE.Mesh(
          this.crumbGeometry,
          this.materials[userUtil.getRandomInt(0, this.materials.length)],
        )
        _c.position.set(
          -i * 10 + userUtil.getRandomFloat(-5, 5),
          0.5,
          20 + userUtil.getRandomFloat(-10, 10),
        )
    ...
    this.candyShape = this.ammo.createShapeFromBuffergeometry(
      this.candyGeometry,
    )
    ...
    userUtil.loadShader(
      'js/shaders/lavaVertex.vert',
      'js/shaders/lavaFragment.frag',
      (vert_text, frag_text) => {
        this.onShaderLoaded(vert_text, frag_text)
      },
    )

    // ------- Lazy Dude -------
    this.lazyDude = new LazyDude(
      this.ammo,
      this.modelLoader,
      './assets/models/lazyDude.glb',
      'lazyDude',
      () => {
        if (this.debugMode) {
    ...
    this.volleyBallShape = this.ammo.createShapeFromBuffergeometry(
      this.volleyBallGeo,
    )
  ...
  cameraInSceneB() {
    ...
    let followPosition = this.cameraController.getFollowPosition()
    TweenMax.to(this.cameraController, 2, {
      setX: followPosition.x - 87,
      setY: 100,
      setZ: 90,
      currentFrustumSize: 150,
      onUpdate: () => {
        this.cameraController.setFrustumSize(
          this.cameraController.currentFrustumSize,
        )
  ...
  onLoadModel(gltf) {
    ...
    this.softBellyGeometry.translate(
      bodyPosition.x,
      bodyPosition.y,
      bodyPosition.z,
    )
    this.ammo.processGeometryForSoftVolume(this.softBellyGeometry)
    this.softBelly = new THREE.Mesh(
      this.softBellyGeometry,
      new THREE.MeshLambertMaterial({ color: 0xff917b }),
    )
  ...
  update(delta) {
    ...
    if (this.pickedCrumbs.length > 0) {
      ...
      let baseHeightVal =
        this.thingsOnHeadHeight / (this.pickedCrumbs.length - 1)
      for (let i = 1; i < this.pickedCrumbs.length; i++) {
        // let timeVariable = (this.time-i);
        let timeVariable = this.time
        this.pickedCrumbs[i].position.z =
          -0.8 -
          ((Math.sin((this.time - i) * 4) + 1) / 2) *
            (baseHeightVal * i * this.crumbHeightFactor)
      }
    }
    ...
    if (this.pickedBalls.length > 0) {
      ...
      let baseHeightVal =
        this.thingsOnHeadHeight / (this.pickedBalls.length - 1)
      for (let i = 1; i < this.pickedBalls.length; i++) {
        let timeVariable = this.time
        this.pickedBalls[i].position.y =
          2 +
          ((Math.sin((this.time - i) * 4) + 1) / 2) *
            (baseHeightVal * i * this.crumbHeightFactor)
      }
  ...
  bePicked(pickedCrumb) {
    ...
    pickedCrumb.grabOffset.addVectors(
      pickedCrumb.position,
  ...
  bePickedFinal(pickedCrumb) {
    ...
    if (pickedCrumb.tag == 'crumb') {
      ...
      if (this.pickedCrumbs.length == 1) {
        ...
        this.tmpVector3.set(
          userUtil.getRandomFloat(-0.15, 0.15),
          userUtil.getRandomFloat(-0.15, 0.15),
          -0.8,
        )
  ...
  throwCrumb() {
    ...
    // throw to target
    let crumbBody = this.ammo.createRigidBody(
      _crumb,
      this.crumbShape,
      5,
      _crumb.position,
    ...
    _crumb.userBtVector3 = new Ammo.btVector3(
      targetPosition.x,
      targetPosition.y,
      targetPosition.z,
    )
    ...
    if (this.thrownCrumbCount > 1 && this.lazyDude.fsm.state == 'idle') {
      ...
      this.thrownCrumbCount > 2 &&
      this.lazyDude.fsm.state == 'confuse'
    ) {
      this.lazyDude.aware()
    } else if (
      this.thrownCrumbCount > 3 &&
      this.lazyDude.fsm.state == 'struggle'
    ) {
      this.lazyDude.flip()
    }
  }
  ...
  throwBall() {
    ...
    // throw to target
    let bBody = this.ammo.createRigidBody(
      _ball,
      this.volleyBallShape,
      5,
      _ball.position,
      _ball.quaternion,
    )
    ...
    targetPosition.multiplyScalar(70)
    _ball.userBtVector3 = new Ammo.btVector3(
      targetPosition.x,
      targetPosition.y,
      targetPosition.z,
    )
  ...
  dropCandy() {
    ...
    if (this.lazyDude.fsm.state == 'flip') {
      ...
      else this.lazyDude.rightEyeAnchor.getWorldPosition(targetPosition)
    } else {
      this.lazyDude.headJoint.getWorldPosition(targetPosition)
      targetPosition.y += 50
    }
    ...
    let candyBody = this.ammo.createRigidBody(
      _candy,
      this.candyShape,
      5,
      targetPosition,
  ...
  createVolleyBall(b_position, returnBall = false) {
    ...
    v_ball.position.copy(b_position)
    userUtil
