export default class Crumb extends THREE.Object3D
{
	...
	createMesh()
	{
		this.mesh = new THREE.Mesh(this.chapter.crumbGeometry, this.chapter.getRandomMaterial());
	...
	createBody()
	{
		if(this.index==0)
		{
			...
			this.body = this.ammo.createRigidBody(this.mesh, this.chapter.crumbShape, 10, this.mesh.position, this.mesh.quaternion);
		...
		else
		{
			...
			// create joint
			this.ammo.createConeConstraint( this.body, this.chapter.crumbs[this.index-1].body, this.piviots[0], this.chapter.crumbs[this.index-1].piviots[1]);
		}
	}
