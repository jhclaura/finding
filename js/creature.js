...
export default class Creature extends THREE.Object3D
{
	...
	createHead()
	{
		...
		if(this.startGrey)
		{
			this.head = new THREE.Mesh( this.creator.getGeometryByType(this.hType), this.creator.materials[this.creator.materials.length-1] );
			this.head.finalMaterial = this.creator.getRandomMaterial();
		}
		else
		{
			this.head = new THREE.Mesh( this.creator.getGeometryByType(this.hType), this.creator.getRandomMaterial() );
		}
		...
		if(this.verticle)
			...
			headPivot.z -= (this.creator.baseRadius+this.jointDistance);
