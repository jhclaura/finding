...
export default class Cha extends THREE.Object3D {
    ...
    this.fsm = new StateMachine({
      ...
      methods: {
        ...
        onDrop: () => {
          ...
          TweenMax.to(this.spineJoint.children[0].quaternion, 0.5, {
            x: 0,
            y: 0,
            z: 0,
            w: 1,
  ...
  onLoadModel(gltf) {
    ...
    this.model.children[0].children[0].children[0].children[2].children[0].children[0].attach(
      this.dummyHead,
    )
    ...
    for (let i = 0; i < this.animations.length; i++) {
      ...
      ] = this.animationMixer.clipAction(this.animations[i]) //this.animations[i].optimize()
      // else
  ...
  activateAllActions(initialAction) {
    for (let key in this.actionDictionary) {
      ...
      else this.setActionWeight(this.actionDictionary[key], 0)
  ...
  prepareCrossFade(startAction, endAction, duration) {
    ...
    this.currentAction = userUtil.getKeyByValue(
      this.actionDictionary,
      endAction,
    )
  }
  ...
  update(delta) {
    ...
    switch (this.fsm.state) {
      ...
      case 'climbing':
        ...
        if (
          this.model.position.distanceToSquared(this.moveTarget) <
          1.5 * 1.5
        ) {
    ...
    let intersect = this.collisionRaycaster.intersectObjects(
      this.scene.children,
    )
  ...
  updateHeadLookAt(raycaster) {
    ...
    if (intersects.length > 0) {
      ...
      this.headLookTarget = userUtil.quaternionLookAt(
        this.neckJoint,
        intersects[0].point,
      )
    ...
    switch (object.tag) {
      ...
      case 'vball':
        ...
        if (this.model.position.distanceToSquared(object.position) > 3 * 3) {
          this.rotateTarget = userUtil.quaternionLookAt(
            this.model,
            object.position,
          )
