...
export default class ChapterManager {
  ...
  setup() {
    ...
    for (let i = 0; i < this.chapterNames.length; i++) {
      ...
      triggerMesh.position.set(
        Math.sin(((360 / this.chapterNames.length) * i * Math.PI) / 180) * 25,
        2.5,
        Math.cos(((360 / this.chapterNames.length) * i * Math.PI) / 180) * 25,
      )
  ...
  start(chapter) {
    switch (chapter) {
      case 'lazy':
        this.currentChapter = new LazyChapter(
          this.ammo,
          this.modelLoader,
      ...
      case 'same':
        this.currentChapter = new SameChapter(
          this.ammo,
          this.modelLoader,
          this.creatureCreator,
        )
      ...
      case 'doubtful':
        this.currentChapter = new DoubtfulChapter(
          this.ammo,
          this.modelLoader,
          this,
        )
